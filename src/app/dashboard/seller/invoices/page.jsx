// app/dashboard/seller/invoices/page.jsx
"use client";
...
export default function SellerInvoicesPage() {
  ...
  // Filter invoices for this seller (mock: assume seller is "Shenzhen Tech Co., Ltd.")
  const sellerInvoices = mockInvoices.filter(
    (inv) => inv.seller.name === "Shenzhen Tech Co., Ltd.",
  );
  ...
  const totalEarned = sellerInvoices.reduce((acc, inv) => acc + inv.total, 0);
  const paidTotal = sellerInvoices
    .filter((inv) => inv.status === "paid")
    .reduce((acc, inv) => acc + inv.total, 0);
  const unpaidTotal = sellerInvoices
    .filter((inv) => inv.status !== "paid")
    .reduce((acc, inv) => acc + inv.total, 0);
  ...
  return (
    <SellerLayout>
      <div>
        ...
        <p className="text-gray-600 mb-6">Manage your sales invoices</p>
                  ...
                  >
                    ...
                    <td className="p-3">
                      <div className="flex gap-2">
                        <Link
                          href={`/dashboard/seller/invoices/${inv.id}`}
